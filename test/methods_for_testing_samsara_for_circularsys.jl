...
function run(N::Int; number_of_nodes = nothing)
    ...
    # all_data kan no brukes vidare, f.eks. ved å kalle `julia -i`
    all_data
end
...
function demo(N::Int, case)
    ...
    for i in 1:N
        _simulation_step!(case, all_actions)
    end
end

function _simulation_step!(case, all_actions)
...
println("\n************************ 200 *************************")
data1K = run(1000)  # ... med manuellt skrevet noder: a:j
println("\n************************ 1000 ****************************")
data1K = run(1000, number_of_nodes=5)
println("\n************************ 15000 ****************************")
data20K = run(15000, number_of_nodes=100)
#println("\n************************ 500.000 *************************")
